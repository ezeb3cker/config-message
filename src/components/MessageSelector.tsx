...
export function MessageSelector({ messages, onSelectMessage, onUpdateMessage, onCreateMessage, onDeleteMessage }: MessageSelectorProps) {
  ...
  return (
    <div className="space-y-2">
      <Label>Definir Mensagem</Label>
      <button
        type="button"
        onClick={() => setOpen(true)}
        className={cn(
          "w-full flex items-center justify-between rounded-md border border-input bg-input-background px-3 py-2 text-sm transition-colors",
          "hover:bg-accent hover:border-accent-foreground/20",
          "focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
          !selectedMessage && "text-muted-foreground"
        )}
      >
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <h3>Grupo de Mensagens #{index + 1}</h3>
                        <div className="flex items-center gap-2">
                          {selectedMessage?.disparo_id === group.disparo_id && (
